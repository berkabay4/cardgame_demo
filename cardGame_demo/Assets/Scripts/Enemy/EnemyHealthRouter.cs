...
public class EnemyHealthRouter : MonoBehaviour
{
    [Header("Refs (auto if empty)")]
    [SerializeField] private SimpleCombatant self;               // Enemy'in SimpleCombatant'ı
    ...
    void AutoWire()
    {
        ...
        if (!canvasTf)
        {
            if (logWarnings) Debug.LogWarning($"[EnemyHealthRouter:{name}] Canvas not found under Enemy.");
            return;
        }
        if (canvasTf.childCount <= 2)
        {
            if (logWarnings) Debug.LogWarning($"[EnemyHealthRouter:{name}] Canvas needs at least 3 children (DEF, ATK, HP).");
            return;
        }

        var hpChild = canvasTf.GetChild(2);
        hpText = hpChild.GetComponentInChildren<TextMeshProUGUI>(true);
        if (!hpText && logWarnings) Debug.LogWarning($"[EnemyHealthRouter:{name}] TMP on Canvas child[2] not found.");
    }
